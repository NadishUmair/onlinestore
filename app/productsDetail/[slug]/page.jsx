...
export default function Page({ params }) {
  ...
  useEffect(() => {
    // Unwrap params to get the actual `slug`
    const getParams = async () => {
      const resolvedParams = await params;  // Wait for the params Promise to resolve
      setUnwrappedParams(resolvedParams); // Set unwrapped params to state

      const productSlug = resolvedParams?.slug?.trim()?.toLowerCase();
      console.log("productSlug", productSlug);

      if (!productSlug) return;

      const foundProduct = products.find(
        (p) => p.slug.trim().toLowerCase() === productSlug
      );
    ...
    <section className="flex h-auto  md:h-[100vh]   py-[50px]  flex-col justify-end items-center">
      ...
      <div className="h-[80%]   md:w-[60%] md:flex justify-between  ">
        ...
        <div className="p-8 md:w-[50%]  flex flex-col justify-center  md:p-4 transition-transform transform">
          <div>
            <h1 className=" font-extrabold text-gray-800 mb-4">{product.name}</h1>
            ...
            <div className="text-sm py-2 font-light text-gray-800 flex ">
                <span className="text-red-600 line-through mr-2">Rs {product.bPrice} </span>
                <span>Rs {product.price|| "Price: N/A"}</span>
